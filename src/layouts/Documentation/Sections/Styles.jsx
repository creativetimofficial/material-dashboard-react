    ...
    render(){
        return (
            <div>
                ...
                <P>If you go to any of our components you will see that somewhere at the beginning of the file we import <code>withStyles</code> from <code>material-ui</code>, then we import from <code>scr/assets/jss/material-dashboard-react.jsx</code> the correspondig style for that component and at the end of the file, we export that component's class with the <code>withStyles</code> function so that the imported style overrides the <code>material-ui</code> style.</P>
                <P>For a better way of understanding this variables, please take a look inside the <code>src/assets/jss/material-dashboard-react</code>. You will see that all of the styles have a unique name that corresponds with the component name and each of them have a comment above for you to know where the style will be imported. You will also see that we've used some of that styles for <code>src/views/*</code> as well. At the beginning of the file you will see some constants that are beeing used on different components.</P>
            </div>
        );
    }
}
